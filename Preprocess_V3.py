...
def main(SampleInputFolder,OverlayInputFolder,LabelInputFolder,LabelMarkers,image,OutputFolder):
    try:
        ...
        blur = cv2.GaussianBlur(Sample_Img,(105,105),0)
        ...
        for region in regionprops(label_image):
            if region.area >= ReginThreshold:
                ...
                Box_Label = Org_Label_Img.crop(( max(minc-10,0),max(0,minr-10),min(maxc+10,l+19),min(maxr+10,w+19)))
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig(OutputFolder + "BoundingBox_" + image+ "_ker105_RegionThreshold_" +str(ReginThreshold),dpi=300)
    except Exception, e:
